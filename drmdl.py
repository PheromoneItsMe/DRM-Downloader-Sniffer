...
def request_decrkey(wvdfile, ipssh, ilicurl, country_code, pid, selection, channel, live_token):
    ...
    if ilicurl and "player.vimeo.com" in ilicurl:
        print("Resolving Vimeo ExpressPlay License Server...")
        vimeo_resp = requests.get(ilicurl)
        ilicurl = vimeo_resp.text.strip()
        print(f"Resolved ExpressPlay URL: {ilicurl}")
...
def request_custom(wvdfile, ipssh, ilicurl, country_code):
    ...
    if country_code and len(country_code) == 2 and country_code.upper() in allowed_countries:
...
def start_process(command, directory):
    ...
    if process.returncode != 0:
        print(f"Failure to process: {command[0]} exited with non-zero return code ({process.returncode})")
        exit(1)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-res', help='Target resolution (e.g. 1080p, 720p, 4k, 2k, 540p, 360p). Default: best available', required=False)
    ...
    try:
        ...
        if not decr_key or not decr_key.strip():
            print("ERROR: Failed to retrieve decryption key from license server.")
            sys.exit(1)

        key_args = []
        for k in decr_key.strip().splitlines():
            if k.strip():
                key_args.extend(["--key", k.strip()])
        ...
        fmt_spec = "bv,ba"
        if args.res:
            res_val = str(args.res).lower().replace("p", "")
            if res_val in ["4k", "2160"]:
                fmt_spec = "bv[height<=2160]+ba/bv,ba"
            elif res_val in ["2k", "1440"]:
                fmt_spec = "bv[height<=1440]+ba/bv,ba"
            elif res_val.isdigit():
                fmt_spec = f"bv[height<={res_val}]+ba/bv,ba"

        if args.verbose:
            print(f"Downloading Encrypted Audio/Video Files (Format: {fmt_spec}, Concurrency: 16 threads)")
        start_process(["yt-dlp", "-o", "file.%(ext)s", "--allow-unplayable-formats", "-N", "16", "-f", fmt_spec,
                       args.url], working_dir)

        # Dynamically locate downloaded video and audio files
        all_files = os.listdir(working_dir)
        video_files = [f for f in all_files if (f.endswith('.mp4') or 'video' in f.lower()) and not f.startswith('decrypted')]
        audio_files = [f for f in all_files if (f.endswith('.m4a') or 'audio' in f.lower()) and not f.startswith('decrypted')]

        if not video_files:
            print("ERROR: Downloaded video file not found in working directory.")
            sys.exit(1)
        if not audio_files:
            print("ERROR: Downloaded audio file not found in working directory.")
            sys.exit(1)
        ...
        if args.verbose:
            ...
            print(f"Found downloaded audio: {downloaded_audio}")

        if args.verbose:
            print("Decrypting Audio")
        start_process(["mp4decrypt"] + key_args + [downloaded_audio, "decrypted_audio.m4a"], working_dir)
        ...
        start_process(["mp4decrypt"] + key_args + [downloaded_video, "decrypted_video.mp4"], working_dir)
